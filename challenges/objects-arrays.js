...
const universities = [];
graduates.forEach((items) =>{
  universities.push(items.university);
});
universities.sort();
...
const contactInfo = [];
graduates.forEach((items) =>{
  contactInfo.push(`${items.first_name} ${items.email}`);
});
console.log(contactInfo);

/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called unisWithUni that contains them all. This will be an array of objects. Log the result. */
const unisWithUni = [];
universities.forEach((items) =>{
  if(items.includes("Uni")){
    unisWithUni.push(items);
  };
...
zooAnimals.forEach((items) =>{
  displayNames.push(`Name:${items.animal_name}, ${items.scientific_name}`);
});
